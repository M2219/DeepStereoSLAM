...
class ResNetFPNe(nn.Module):
    """
    ResNet18, output resolution is 1/8.
    Each block has 2 layers.
    """
    def __init__(self, input_dim=3, output_dim=256, ratio=1.0, norm_layer=nn.BatchNorm2d, init_weight=True):
        super().__init__()
        # Config
        block = BasicBlock
        ...
        self.layer4 = self._make_layer(block, block_dims[2], stride=2, norm_layer=norm_layer, num=n_block[2])  # 1/16
        self.layer5 = self._make_layer(block, block_dims[2], stride=2, norm_layer=norm_layer, num=n_block[2])  # 1/24
        self.final_conv = conv1x1(block_dims[4], output_dim)
        self._init_weights()
